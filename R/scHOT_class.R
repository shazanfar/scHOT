...
check_validity <- function(object) {
  ...
  if (!is.null(object@positionType)) {
    if (!object@positionType %in% c("trajectory", "spatial") | length(object@positionType) > 1) {
  ...
  if (!is.null(object@positionColData)) {
    if (!all(object@positionColData %in% colnames(colData(object)))) {
      errors <- c("positionColData should be one of the colnames of colData of the scHOT object")
      return(errors)
    }
  }
...
#' @slot positionType A character indicates the type of the position, either trajectory or spatial
#' @slot positionColData A vector indicates column names of colData that stored the postion informaton
#'
